class AsanaService
	class << self
		...
		def post_time_entry_to_task(time_entry)
			...
			return unless client # Token refresh failed
		...
		# Refresh Asana access token if expired or about to expire
		def refresh_token(user)
			return false unless user.asana_refresh_token.present?

			Rails.logger.info("Refreshing Asana token for user #{user.id}")
			...
			if response.success?
				...
				user.update(
					...
					asana_token_expires_at: Time.current + token_data['expires_in'].seconds
				)
				Rails.logger.info("Successfully refreshed Asana token for user #{user.id}")
				true
			else
				Rails.logger.error("Failed to refresh Asana token for user #{user.id}: #{response.code}")
				false
			end
		rescue StandardError => e
			Rails.logger.error("Error refreshing Asana token: #{e.message}")
			false
		end
		...
		def asana_client(user)
			...
			if user.asana_token_expires_at.nil? || user.asana_token_expires_at < 5.minutes.from_now
				Rails.logger.info("Token expired or expiring soon, attempting refresh")
				
				# Try to refresh
				unless refresh_token(user)
					Rails.logger.error("Token refresh failed for user #{user.id}")
					return nil
				end
